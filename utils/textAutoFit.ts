...
export const SAFE_TEXT_ZONES: Record<BubbleType, { widthFactor: number; heightFactor: number }> = {
  ...
  [BubbleType.Thought]: { widthFactor: 0.60, heightFactor: 0.65 },      // Nuages de pensée
...
export function getTextBounds(bubble: Bubble): { width: number; height: number; x: number; y: number } {
  ...
  const textWidth = bubble.width * safeZone.widthFactor;
  const textHeight = bubble.height * safeZone.heightFactor;
  const textX = (bubble.width - textWidth) / 2;
  const textY = (bubble.height - textHeight) / 2;
